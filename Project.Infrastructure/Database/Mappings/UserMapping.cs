...
namespace Project.Infrastructure.Database.Mappings
{
    public class UserEntityConfiguration : EntityCongurationMapper<User>
    {
        public override void Configure(EntityTypeBuilder<User> builder)
        {
            ...
            builder.HasMany(x => x.UserSkillGains).WithOne(c => c.User).HasPrincipalKey(c => c.Id).HasForeignKey(c => c.UserId);
            builder.HasMany(x => x.UserJobApplications).WithOne(c => c.User).HasPrincipalKey(c => c.Id).HasForeignKey(c => c.Id); //Added in new
